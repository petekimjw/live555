   ...
   while (*cursor) {
      if ((cursor[0] == '%') &&
         ...
         *url++ = (char)strtol(hex, NULL, 16);
         ...
         // 일반적인 경우: 이것은 일반 문자이거나 가짜 % 표현식이므로 그냥 복사합니다.
         *url++ = *cursor++;
      }
   }
...
//1. cmdName : RTSP 메서드(예: OPTIONS, DESCRIBE, SETUP, …)
//2. urlPreSuffix / urlSuffix : URL 경로를 마지막 슬래시 기준으로 둘로 나눈 값
//3. CSeq : 필수 헤더
//4. Session: : 선택 헤더(있으면 값 추출)
//5. Content-Length: : 선택 헤더(있으면 값 추출)
   ...
   unsigned& contentLength, Boolean& urlIsRTSPS) 
{
   ...
   // "수락하는 것에 관대하세요": 요청 시작 부분의 공백은 건너뜁니다.
   unsigned i;
   ...
   // "rtsp://" 또는 "rtsp:/"(또는 "rtsps://" 또는 "rtsps:/") 접두사는 건너뜁니다.
   // 다음에 오는 URL:
   unsigned j = i + 1;
   ...
   for (; (int)j < (int)(reqStrSize - 8); ++j) {
      if ((reqStr[j] == 'r' || reqStr[j] == 'R')
         ...
         if (reqStr[j + 4] == ':' && reqStr[j + 5] == '/') {
            j += 6;
            if (reqStr[j] == '/') {
               // 이것은 "rtsp(s)://" URL입니다. 다음에 나오는 host:port 부분은 건너뜁니다.
               ++j;
               while (j < reqStrSize && reqStr[j] != '/' && reqStr[j] != ' ') ++j;
            }
            else {
               // 이것은 "rtsp(s):/" URL입니다. 다시 "/"로 돌아갑니다.
               --j;
            }
            i = j;
            break;
   ...
   for (unsigned k = i + 1; (int)k < (int)(reqStrSize - 5); ++k) {
      if (reqStr[k] == 'R' && reqStr[k + 1] == 'T' &&
         ...
         // ASSERT: 이 지점에서
         // i: "host" 또는 "host:port" 뒤의 첫 번째 공백 또는 슬래시
         // k: "RTSP/" 앞의 마지막 공백 없음
         // k1: [i,k] 범위의 마지막 슬래시

         // URL 접미사는 [k1+1,k]에서 가져옵니다.
         // "resultURLSuffix" 복사:
         ...
         // URL 'pre-suffix'는 [i+1,k1-1]에서 생성됩니다.
         // "resultURLPreSuffix"를 복사합니다.
         n = 0; k2 = i + 1;
         if (k2 + 1 <= k1) {
   ...
   // "CSeq:"(필수, 대소문자 구분 없음)를 찾고 공백을 건너뜁니다.
   // 다음 \r 또는 \n까지의 모든 내용을 'CSeq'로 읽습니다.
   parseSucceeded = False;
   ...
   // "Session:"(선택 사항, 대소문자 구분 없음)을 찾고 공백을 건너뜁니다.
   // 다음 \r 또는 \n까지의 모든 내용을 'Session'으로 읽습니다.
   ...
   // 또한: "Content-Length:"를 찾으세요(선택 사항, 대소문자 구분 없음)
