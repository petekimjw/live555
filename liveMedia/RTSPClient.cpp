...
//fCurrentAuthenticator 이용하여 인증문자열 생성 (Digest or Basic)
char* RTSPClient::createAuthenticatorString(char const* cmd, char const* url)
{
   ...
   if (auth.realm() != NULL && auth.username() != NULL && auth.password() != NULL) 
   {
      ...
      if (auth.nonce() != NULL)
      {
         #pragma region Digest authentication
         ...
         //Authorization: Digest username="user1", realm="LIVE555 Streaming Media", nonce="357cdb76b9aa55b6d82ee9659990fce7", uri="rtsp://user1:pass1@172.16.15.143:8554/h264ESVideoTest", response="d1aa28be22cc139d126b5da198791f9c"
      ...
      {
         #pragma region Basic authentication
   ...
   do {
      ...
      if (streamUsingTCP) {
         ...
         if (subsession.rtcpInstance() != NULL) 
            subsession.rtcpInstance()->setStreamSocket(fInputSocketNum, subsession.rtcpChannelId, fInputTLS);
...
Boolean RTSPClient::checkForHeader(char const* line, char const* headerName, unsigned headerNameLength, char const*& headerParams)
{
   ...
   // 해당 줄은 원하는 헤더 이름으로 시작합니다. 공백을 제거하고 헤더 매개변수를 반환합니다.
   unsigned paramIndex = headerNameLength;
   while (line[paramIndex] != '\0' && (line[paramIndex] == ' ' || line[paramIndex] == '\t')) ++paramIndex;
   if (line[paramIndex] == '\0') return False; // 매개변수가 없으면 헤더가 잘못된 것으로 간주됩니다.
...
{
   if (paramsStr == NULL) return False; //"WWW-Authenticate:" 헤더없음
   ...
   if (sscanf(paramsStr, "Digest realm=\"%[^\"]\", nonce=\"%[^\"]\", algorithm=\"%[^\"]\"", realm, nonce, algorithm) == 3)
   {
      ...
      if (_stricmp(algorithm, "SHA-256") == 0) 
      {
         fCurrentAuthenticator.setUseSHA256(true);//SHA-256
      }
      else 
      {
         fCurrentAuthenticator.setUseSHA256(false);//MD5
      }
   }
   ...
   do
   {
      ...
      do
      {
         ...
         while (1)
         {
            ...
            nextLineStart = getLine(lineStart);
            if (lineStart[0] == '\0') break; // 이것은 빈 줄입니다
         ...
         if (foundRequest != NULL)
         {
            ...
            else if (responseCode == 401 && handleAuthenticationFailure(wwwAuthenticateParamsStr))
            {
               ...
               // "Authorization:" 헤더와 함께 명령을 다시 보내야 합니다.
...
#pragma region RTSPClient::RequestRecord 구현
...
#pragma region RTSPClient::RequestQueue 구현
