...
void RTSPClient::handleResponseBytes(int newBytesRead)
{
   ...
   do
   {
      // 데이터 읽기가 정상적으로 완료되었습니다. 지금까지 읽은 데이터를 검토하여 <CR><LF><CR><LF>가 포함되어 있는지 확인하세요.
      // (포함되어 있지 않으면 추가 데이터가 도착할 때까지 기다리세요.)
      ...
      // 이제 완전한 응답 헤더(<CR><LF><CR><LF>로 끝남)를 얻었으므로, 응답 코드, CSeq,
      // 및 기타 다양한 헤더 매개변수를 얻기 위해 파싱합니다. 이를 위해 먼저 수신된 헤더 데이터의 복사본을 만듭니다.
      // '\0' 바이트를 추가하여 수정할 것이기 때문입니다.
      char* headerDataCopy;
      ...
      do
      {
         ...
         if (!parseResponseCode(lineStart, responseCode, responseStr)) {
            // 이것은 RTSP 응답이 아닌 것 같습니다. 아마도 RTSP 요청일 수도 있겠죠?
      ...
      #pragma region 응답에 대한 Handler 콜백호출
