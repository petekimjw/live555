...
#pragma region Send (Describe, Option, Play, Pause, Record, Teardown, Parameter 등)
...
// PLAY의 "속도:" 옵션을 사용하여 더 빠른 다운로드 지원 추가
...
unsigned RTSPClient::sendRequest(RequestRecord* request) 
{
   ...
   do {
      ...
      if (!fRequestsAwaitingConnection.isEmpty()) {
         // 연결이 현재 보류 중입니다(최소 하나의 요청이 대기 중입니다). 다음 요청도 대기열에 추가하세요.
      ...
      // 요청이 있는 경우(아직 수행 중이 아니거나 수행한 적이 없는 경우), RTSP-over-HTTP 터널링을 위한 특수 프로토콜을 설정합니다.
      ...
      #pragma region 명령별 헤더 구성(Accept, Content-Type, Transport, Session, Blocksize, KeyMgmt 등)
      ...
      //Send명령의 필드를 설정(Accept, Content-Type, Transport, Session, Blocksize, KeyMgmt 등)
      ...
      //Authorization: Digest username="user1", realm="LIVE555 Streaming Media", nonce="357cdb76b9aa55b6d82ee9659990fce7", uri="rtsp://1@172.16.15.143:8554/h264ESVideoTest", response="d1aa28be22cc139d126b5da198791f9c"
      ...
      {   
         // RTSP-over-HTTP 터널링을 수행할 때, 요청을 전송하기 전에 Base-64 인코딩을 수행합니다.
         // (단, 터널 설정에 사용하는 HTTP "GET" 및 "POST" 명령에는 이 작업을 수행하지 않습니다.)
         char* origCmd = cmd;
      ...
      // 명령 전송이 성공했으므로 요청 레코드를 큐에 추가하여 응답(오류 발생 시)을 처리할 수 있도록 합니다.
      // 하지만 RTSP-over-HTTP를 사용하는 POST 명령에 대한 응답은 예상하지 않으므로 큐에 추가하지 마세요.
   ...
   // 오류가 발생했으므로 응답 핸들러를 즉시 호출합니다(오류를 나타냄):
   delete[] cmd;
...
//Send명령의 필드를 설정(Accept, Content-Type, Transport, Session, Blocksize, KeyMgmt 등)
Boolean RTSPClient::setRequestFields(RequestRecord* request, char*& cmdURL, Boolean& cmdURLWasAllocated, char const*& protocolStr, char*& extraHeaders, Boolean& extraHeadersWereAllocated) 
{
   ...
   {
      // 현재 세션에 참여 중이라면 "Session:" 헤더를 생성합니다
      // (서버가 클라이언트의 '활성 상태'를 나타내길 원할 경우). 이는 '추가 헤더'를 구성합니다.
...
#pragma region resetTCPSockets, resetResponseBuffer
...
#pragma region openConnection, connectToServer
...
char* RTSPClient::createAuthenticatorString(char const* cmd, char const* url) {
   ...
   if (auth.realm() != NULL && auth.username() != NULL && auth.password() != NULL) {
      ...
      if (auth.nonce() != NULL) // Digest authentication
      {
         ...
         {
            if (strcmp(auth.algorithm(), "SHA-256") == 0)
               auth.setUseSHA256(true);
            else
               auth.setUseSHA256(false);

            char const* const authFmt = "Authorization: Digest username=\"%s\", realm=\"%s\", "
               "nonce=\"%s\", uri=\"%s\", algorithm=%s, response=\"%s\", opaque=\"%s\", "
               "qop=\"%s\", nc=%s, cnonce=\"%s\"\r\n";
            ...
            unsigned authBufSize = authFmtSize + strlen(auth.username()) + strlen(auth.realm())
               + strlen(auth.nonce()) + strlen(auth.opaque()) + strlen(auth.cnonce()) + strlen(url)
               + strlen(auth.nc()) + strlen(response) + strlen(auth.algorithm() + strlen(auth.qop()));

            authenticatorStr = new char[authBufSize];

            sprintf(authenticatorStr, authFmt, auth.username(), auth.realm(),
               auth.nonce(), url, auth.algorithm(), response, auth.opaque(),
...
#pragma region handleSETUPResponse
...
#pragma region handle (RequestError, SetupResponse, PlayResponse, handleResponseBytes 등)
...
Boolean RTSPClient::handlePLAYResponse(MediaSession* session, MediaSubsession* subsession, char const* scaleParamsStr, char const* speedParamsStr, char const* rangeParamsStr, char const* rtpInfoParamsStr) 
{
...
#pragma region handleAuthenticationFailure

#define SKIP_WHITESPACE while (*fields != '\0' && (*fields == ' ' || *fields == '\t')) ++fields

static Boolean parseAuthorizationHeader(char const* buf, char const*& realm, char const*& nonce, char const*& opaque, char const*& algorithm, char const*& qop, char const*& stale)
{
   // Initialize the result parameters to default values:
   realm = nonce = opaque = algorithm = qop = stale = NULL;

   // First, find "Authorization:"
   while (1) {
      if (*buf == '\0') return False; // not found
      if (_strncasecmp(buf, "Digest ", 7) == 0) break;
      ++buf;
   }
   ...
   do
   {
      ...
      *p = '\0'; // complete parsing <parameter>
      ...
      for (p = value; *fields != '\0'/* && (quotes && *fields != '"')*/;) {
         if (quotes) {
            if (*fields == '"') break;
         }
         else {
      ...
      if (quotes && *fields++ != '"') break; // parsing failed
      *p = '\0'; // complete parsing <value>
      SKIP_WHITESPACE;
      success = True;

      // Copy values for parameters that we understand:
      if (strcmp(parameter, "realm") == 0) {
         realm = strDup(value);
      }
      else if (strcmp(parameter, "nonce") == 0) {
         nonce = strDup(value);
      }
      else if (strcmp(parameter, "opaque") == 0) {
         opaque = strDup(value);
      }
      else if (strcmp(parameter, "algorithm") == 0) {
         algorithm = strDup(value);
      }
      else if (strcmp(parameter, "qop") == 0) {
         qop = strDup(value);
      }
      else if (strcmp(parameter, "stale") == 0) {
         stale = strDup(value);
      }
...
//"WWW-Authenticate:" 헤더에서 fCurrentAuthenticator 객체파싱 -> 재시도하려면 True, 인증실패면 False 반환
Boolean RTSPClient::handleAuthenticationFailure(char const* paramsStr)
{
   ...
   char const* opaque = NULL;
   char const* algorithm = NULL;
   char const* qop = NULL;
   char const* stale = NULL;
   Boolean success = True;

   if (!parseAuthorizationHeader(paramsStr,
      realm, nonce, opaque, algorithm, qop, stale)) {
      return false;
   }

   if (realm && nonce && opaque && algorithm && qop) {
   ...
   else if (realm && nonce && algorithm) {
      realmHasChanged = fCurrentAuthenticator.realm() == NULL || strcmp(fCurrentAuthenticator.realm(), realm) != 0;
      fCurrentAuthenticator.setRealmAndNonce(realm, nonce);
      algorithmHasChanged = fCurrentAuthenticator.algorithm() == NULL || strcmp(fCurrentAuthenticator.algorithm(), algorithm) != 0;
      if (algorithmHasChanged) {
         fCurrentAuthenticator.setOpaqueAndAlgorithmAndQop(NULL, algorithm, NULL);
      }
   }
   else if (realm && nonce && stale) {
      realmHasChanged = fCurrentAuthenticator.realm() == NULL || strcmp(fCurrentAuthenticator.realm(), realm) != 0;
   ...
   if (success) {
      if ((!realmHasChanged && !isStale) || fCurrentAuthenticator.username() == NULL || fCurrentAuthenticator.password() == NULL) {
         // 이미 동일한 영역(그리고 오래된 nonce가 아닌)으로 시도했거나,
         // 사용자 이름 및/또는 비밀번호가 없는 경우, 새로운 "WWW-Authenticate:" 헤더
         // 정보는 도움이 되지 않습니다. 인증되지 않은 상태로 유지됩니다.
         success = False;
      }
   }
...
#pragma region 연결처리 (connectionHandler)

//TCP 연결 완료 시점에 수행하는 후처리 핸들러.
//연결 상태를 확인해 성공이면 대기 중이던 RTSP 요청들을 전송 재개하고, 실패면 모두 오류로 마무리
...
{
   //소켓이 준비되면 동작하는 핸들러 등록 (incomingDataHandler)
   envir().taskScheduler().disableBackgroundHandling(fOutputSocketNum);
   envir().taskScheduler().setBackgroundHandling(fInputSocketNum, SOCKET_READABLE | SOCKET_EXCEPTION,
      (TaskScheduler::BackgroundHandlerProc*)&incomingDataHandler, this);

   // 연결을 기다리는 모든 요청을 새로운 임시 큐로 옮겨 "fRequestsAwaitingConnection"을 비웁니다.
   // ("fRequestsAwaitingConnection"이 비어 있지 않아 "sendRequest()"가 혼동되어 다시 큐에 넣지 않도록 합니다.)
   RequestQueue tmpRequestQueue(fRequestsAwaitingConnection);
   RequestRecord* request;

   // 연결이 성공했는지 실패했는지 확인합니다.
   do 
   {
      int err = 0;
      SOCKLEN_T len = sizeof err;
      // 참고: 일반적으로 "fOutputSocketNum" == "fInputSocketNum"입니다. 단, RTSP-over-HTTP를 수행할 때 두 번째(즉, "POST") 연결에 연결할 때는 예외입니다.
      //getsockopt(SO_ERROR)로 커넥션 에러 확인
      ...
      // 참고: 일반적으로 "fOutputTLS" == "fInputTLS"입니다. 단, RTSP-over-HTTP를 사용할 때 두 번째(즉, "POST") 연결에 연결할 때는 예외입니다.
      if (fOutputTLS->isNeeded) 
      {
         // 추가 TLS 연결을 완료해야 합니다.
      ...
      // 연결이 성공했습니다. RTSP-over-HTTP 설정 시도로 연결이 이루어진 경우, 지금 다음 작업을 완료하세요.
      if (fHTTPTunnelingConnectionIsPending && !setupHTTPTunneling2()) break;

      // 연결이 완료되었습니다. 보류 중인 모든 요청 전송을 재개합니다.
   ...
   //오류가 발생했습니다. 보류 중인 모든 요청에 오류를 알립니다
   resetTCPSockets(); // 오류 처리기가 "this"를 삭제하는 경우를 대비해 지금 이 작업을 수행합니다.
...
#pragma region 수신처리 (incomingDataHandler, handleResponseBytes)
...
//RTSP 응답파싱후 해당 명령(DESCRIBE, SETUP, PLAY 등)에 맞는 콜백을 호출
//1. fResponseBuffer(응답 누적 버퍼) : RTSP TCP 소켓으로부터 새로 받은 바이트 배열 누적
//2. 헤더파싱(CSeq, Content-Base, Session, Transport, Scale, WWW-Authenticate 등)
//3. RTSP 응답코드에 따라 적절한 핸들러 호출(SetupResponse, PlayResponse, handleRequestError 등)
//4. 응답에 대한 Handler 콜백호출 (foundRequest->handler())
...
void RTSPClient::handleResponseBytes(int newBytesRead)
{
   ...
   do 
   {
      ...
      if (newBytesRead >= (int)fResponseBufferBytesLeft) {
         // 응답 버퍼가 가득 찼습니다. (첫 번째 응답 핸들러에 대한) 오류로 처리합니다.
         envir().setResultMsg("RTSP response was truncated. Increase \"RTSPClient::responseBufferSize\"");
      }

      // TCP 소켓을 읽는 중 오류가 발생했습니다. 보류 중인 모든 응답 핸들러를 호출하여 이 오류를 표시합니다.
      // (단, "RTSP 응답이 잘렸습니다" 오류는 첫 번째 응답 핸들러에만 적용됩니다.)
      resetResponseBuffer();
      RequestRecord* request;
   ...
   do 
   {
      #pragma region 읽은 데이터 안에 <CR><LF><CR><LF>가 포함확인, 포함되어 있지 않으면 추가 데이터가 대기
      ...
      if (!endOfHeaders) return; // 완전한 응답을 얻으려면 후속 읽기가 필요합니다.

      // 이제 완전한 응답 헤더(<CR><LF><CR><LF>로 끝남)를 얻었으므로, 
      // 응답 코드, CSeq 및 기타 헤더 파싱
      #pragma region 헤더 데이터의 복사본 사용('\0' 바이트를 추가하여 수정)
      char* headerDataCopy;
      ...
      do 
      {
         ...
         } while (lineStart[0] == '\0' && nextLineStart != NULL); // 시작 부분의 빈 줄을 건너뜁니다.  
         #pragma endregion
         ...
         #pragma region 헤더파싱(CSeq, Content-Base, Session, Transport, Scale, WWW-Authenticate 등)
         ...
         while (1) {
            reachedEndOfHeaders = True; // 기본적으로; 아래에서 변경될 수 있음
            lineStart = nextLineStart;
            ...
            if (checkForHeader(lineStart, "CSeq:", 5, headerParamsStr)) {
               ...
               RequestRecord* request;
               #pragma region CSeq에 대한 핸들러 함수를 찾기

               while ((request = fRequestsAwaitingResponse.dequeue()) != NULL) 
               {
                  if (request->cseq() < cseq) 
                  {
                     // CSeq 카운터가 절대 래핑되지 않는다고 가정합니다.
                     // 이 핸들러에 대한 응답을 받지 못했고 앞으로도 받지 않을 것이므로 삭제합니다.
                     ...
                     // 이것이 우리가 원하는 핸들러입니다. 해당 레코드를 삭제하되, 나중에 핸들러를 호출할 수 있도록 기억해 두세요.
                     foundRequest = request;
                     break;
                  }
                  else { // request->cseq() > cseq
                     // 이 응답에 대한 핸들러가 등록되지 않았으므로 무시합니다.
                     break;
                  }
               }
            ...
            {
               //이미 "WWW-Authenticate:" 헤더를 본 적이 있다면, 새 헤더가 "Digest" 인증을 지정하는 경우에만새 헤더로 바꿉니다.
            ...
            {
               // 참고: "공개:" 대신 "허용:"을 허용하므로 HTTP 서버에 대한 "옵션" 요청이 작동합니다.
            }
            ...
            {
               // '기본 URL'의 끝부분을 이 헤더 매개변수 값으로 바꿉니다.
               ...
               if (oldBaseURLTail != NULL) {
                  ...
                  // 참고: 일부 컴파일러가 지원하지 않기 때문에 "asprintf()"를 사용할 수 없습니다.
         ...
         if (foundRequest == NULL) {
            // 해킹: 응답에 "CSeq:" 헤더가 없습니다. 가장 최근 요청에 대한 것이라고 가정합니다.
            foundRequest = fRequestsAwaitingResponse.dequeue();
         }
         #pragma endregion

         #pragma region "Content-Length:" 헤더가 있는 경우, 지정된 양의 데이터가 있는지 확인하세요. 없으면 추가 데이터 읽기를 대기
         ...
         {
            // 더 많은 데이터를 읽어야 합니다. 먼저, 충분한 공간이 있는지 확인하세요.
         ...
         // 이제 완전한 응답이 있습니다("Content-Length:" 헤더에 지정된 모든 바이트 포함, 있는 경우).
         ...
         fCurrentAuthenticator.updateCount();
         if (foundRequest != NULL)
         {
            #pragma region CSeq에 대응되는 응답처리(handleSETUPResponse, handlePLAYResponse 등)

            if (responseCode == 200)
            {
               #pragma region 성공응답(200 OK, Setup, Play, Teardown, Get_Parameter)의 처리수행
            ...
            {
               #pragma region 인증방식에(사용자정보, MD5, SHA-256등) 따라 재전송

               needToResendCommand = True;

               if (strcmp(foundRequest->commandName(), "GET") == 0) 
               {
                  // 참고: HTTP "GET" 명령(RTSP-over-HTTP 터널링용)이 "401 Unauthorized"를 반환하면, RTSP 명령과 마찬가지로
                  // "Authorization:" 헤더와 함께 해당 명령을 다시 전송합니다. 단, 이 과정에서는 새로운 TCP 연결을 사용합니다.
                  // 일부 서버는 "401 Unauthorized"를 반환한 후 원래 연결을 닫기 때문입니다.
                  resetTCPSockets(); // 재전송된 명령에 대한 새 연결을 강제로 엽니다.
               }
            ...
            { // redirection
               resetTCPSockets(); // 다음에 다른 곳에 연결해야 하기 때문에
               needToResendCommand = True;
            }
            ...
            if (needToResendCommand) 
            {
               ...
               delete[] headerDataCopy;
               return; // 응답 핸들러를 호출하지 않고 다시 전송된 명령에 대한 응답이 이를 수행합니다.
            }
            #pragma endregion
         }
      ...
      #pragma region 응답에 대한 Handler 콜백호출 (foundRequest->handler())

      // 이 응답에 대한 핸들러 함수가 있으면 호출합니다.
      // 하지만 먼저, 핸들러가 이벤트 루프로 이동하여 재귀적으로 호출될 경우를 대비하여 응답 버퍼를 재설정합니다.
      if (numExtraBytesAfterResponse > 0) 
      {
         // 특이한 경우입니다. 일반적으로 파이프라인 응답을 받았기 때문입니다. 추가 바이트를 버퍼의 앞으로 이동합니다.
         char* responseEnd = &fResponseBuffer[fResponseBytesAlreadySeen - numExtraBytesAfterResponse];

         // 하지만 먼저: "resultString"에 대해 필요할 경우를 대비해 응답 '본문'의 사본을 저장하는 방법을 알려드리겠습니다.
      ...
      if (foundRequest != NULL && foundRequest->handler() != NULL) {
         ...
         if (responseSuccess) {
            if (responseCode == 200) {
               ...
               // 참고: "strDup(bodyStart)" 호출은 본문이 내부 '\0' 바이트 없이 인코딩되었다고 가정합니다.
            }
            else {
            ...
            // 응답을 구문 분석하는 동안 오류가 발생했으므로 오류를 나타내는 핸들러를 호출합니다.
...
//RTSP 요청 문자열에서 cmdName, urlPreSuffix, urlSuffix, CSeq, Session, Content-Length 항목파싱
void RTSPClient::handleIncomingRequest() 
{
   // 요청 문자열을 명령 이름과 'CSeq'로 구문 분석한 다음, 명령을 '처리'합니다(지원하지 않는다는 응답을 통해).
...
//문자열(line)에서 RTSP/%*s%u 또는 HTTP/%*s%u 형식의 응답코드 파싱
Boolean RTSPClient::parseResponseCode(char const* line, unsigned& responseCode, char const*& responseString) {
   ...
   // 참고: RTSP-over-HTTP 터널링을 설정하기 위해 HTTP 응답과 RTSP 응답을 모두 확인합니다.
   // 클라이언트가 실수로 HTTP 전용 서버로 RTSP 명령을 전송하려고 시도한 경우 의미 있는 오류를 반환하기 위해서입니다.

   // RTSP/*(또는 HTTP/*) 뒤의 모든 내용을 응답 문자열로 사용합니다.
   responseString = line;
...
#pragma region (이미 설정된) 연결을 통해 데이터 쓰기/읽기:
...
#pragma region RequestRecord 구현
...
#pragma region HandlerServerForREGISTERCommand 구현
